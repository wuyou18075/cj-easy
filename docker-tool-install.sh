...
_install_service() {
    ...
    if echo "${SVC_KEYS[*]}" | grep -qw "new-api" || [ "$NAME" = "New API" ]; then
        ...
        echo -e "     ${C_YELLOW}docker exec -it postgres psql -U <用户名> -d <已有库> -c \"CREATE DATABASE new_api;\"${C_RESET}"
        echo -e "     示例(用户 cj、已有库 test):"
        echo -e "     ${C_YELLOW}docker exec -it postgres psql -U cj -d test -c \"CREATE DATABASE new_api;\"${C_RESET}"
        echo -e "  3) SQL_DSN 格式: ${C_GREEN}postgresql://用户名:密码@主机:端口/数据库名${C_RESET}"
        echo -e "     默认模板: postgresql://postgres:postgres@host.docker.internal:5432/new_api"
        echo -e "     含义: 协议://用户:密码@host.docker.internal:5432/库名"
        echo -e "     你的账号若不是 postgres，请改成实际用户和密码，例如:"
        echo -e "     ${C_YELLOW}postgresql://cj:你的密码@host.docker.internal:5432/new_api${C_RESET}"
        echo -e "  4) REDIS 格式: ${C_GREEN}redis://:密码@主机:端口${C_RESET}"
        echo -e "     默认模板: redis://:redis@host.docker.internal:6379"
        echo -e "     密码不同请改 :密码@ 这一段"
        echo -e "  5) 库已存在会报 already exists，可忽略，直接继续安装"
    fi
    echo -e "${LINE_GRAY}"
